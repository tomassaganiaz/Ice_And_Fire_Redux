...
/** Reina Myrmex — genera huevos y dirige la colonia */
...
public class EntityMyrmexQueen extends EntityMyrmexBase {
    ...
    public void onLivingUpdate() {
        ...
        if(hasMadeHome() && this.getGrowthStage() >= 2 && !this.canSeeSky()) {
            ...
            updateHiveCreation();
            if(this.getAnimation() == ANIMATION_DIGNEST && this.getAnimationTick() == 42){
                return;
            }
        }
        updateEggLaying();
    ...
    private void updateHiveTick() {
        if(this.getHive() != null){
            this.getHive().tick(0, world);
        }
    }

    private void updateHiveCreation() {
    ...
    private void updateEggLaying() {
        ...
        boolean shouldLay = eggTicks > IceAndFireConfig.ENTITY_SETTINGS.myrmexPregnantTicks && (
            this.getHive() == null || this.getHive().repopulate()
        );
        if(!shouldLay) return;
        float radius = -5.25F;
        ...
        if(!world.isAirBlock(eggPos)) return;
    ...
    private void updateBiteAttack() {
        if (this.getAnimation() != ANIMATION_BITE || this.getAttackTarget() == null || this.getAnimationTick() != 6) return;
        this.playBiteSound();
    ...
    private void updateStingAttack() {
        if (this.getAnimation() == ANIMATION_STING && this.getAnimationTick() == 0){
            this.playStingSound();
        }
        if (this.getAnimation() != ANIMATION_STING || this.getAttackTarget() == null || this.getAnimationTick() != 6) return;
        if (!this.getAttackBounds().intersects(this.getAttackTarget().getEntityBoundingBox())) return;
