...
/** Serpiente marina — depredador oceánico */
...
public class EntitySeaSerpent extends EntityAnimal implements IAnimatedEntity, IMultipartEntity, IVillagerFear, IAnimalFear {
    ...
    public void onLivingUpdate() {
        ...
        updateJumpArc();
        updateSwimCycle();
        updateBreathProgress(breathing);
        updateJumpProgress(jumping, wantJumping);
        updateJumpRotation();
        handleSplashDown(prevJumping);
        updateNavigator(ground);
        updateRotation();
        updateClientBuffers(jumping);
        updateServerCombat();
        updateBiteAttack();
        breakBlock();
        handleBoatRiding();
    ...
    private void updateJumpState() {
        if (!world.isRemote && isJumpingOutOfWater() && swimBehavior == SwimBehavior.WANDER && shouldStopJumping()) {
            motionY -= 0.25D;
            if (this.isInWater()) {
    ...
    private void updateBreathProgress(boolean breathing) {
    ...
    private void updateJumpProgress(boolean jumping, boolean wantJumping) {
    ...
    private void handleSplashDown(boolean prevJumping) {
    ...
    private void updateNavigator(boolean ground) {
    ...
    private void updateClientBuffers(boolean jumping) {
        if (world.isRemote) {
