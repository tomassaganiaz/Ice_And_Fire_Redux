...
/** Hipocampo — criatura acuática montable */
...
public class EntityHippocampus extends EntityTameable implements IAnimatedEntity, IDropArmor {
    ...
    private void applyWaterBreathingToRider() {
        if (getControllingPassenger() instanceof EntityLivingBase && this.ticksExisted % 20 == 0) {
    ...
    private void handleNavigatorSwitching() {
    ...
    private void syncArmorToServer() {
        if (!world.isRemote || hippocampusInventory == null) return;
    ...
    public void travel(float strafe, float vertical, float forward) {
        ...
        if (this.isBeingRidden() && this.canBeSteered()) {
            travelRidden(strafe, vertical, forward);
            return;
        }
        travelAI(strafe, vertical, forward);
        updateLimbSwing();
    }

    private void travelRidden(float strafe, float vertical, float forward) {
        ...
        this.fallDistance = 0;
        if (this.isInWater()) {
            travelRiddenWater(strafe, vertical, forward);
        } else {
            travelRiddenLand(strafe, forward);
        }
    }

    private void travelRiddenWater(float strafe, float vertical, float forward) {
        this.moveRelative(strafe, vertical, forward, 1F);
        float f4 = 0.8F;
        float d0 = 3;
        if (!this.onGround) d0 *= 0.5F;
        if (d0 > 0.0F) f4 += (0.54600006F - f4) * d0 / 3.0F;
        this.motionX *= f4 * 0.900000011920929D;
        this.motionY *= f4 * 0.900000011920929D;
        this.motionZ *= f4 * 0.900000011920929D;
        motionY += 0.01185D;
        ...
        updateLimbSwing();
        if (limbSwingAmount > 0.5D && this.isInWater() && this.isBeingRidden()) {
            this.doWaterSplashEffect();
        }
    }

    private void travelRiddenLand(float strafe, float forward) {
        forward *= 0.25F;
        strafe *= 0.125F;
        this.setAIMoveSpeed(onGround ? (float) this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getAttributeValue() : 1);
        super.travel(strafe, 0, forward);
    ...
    private void travelAI(float strafe, float vertical, float forward) {
        if (!this.isServerWorld()) return;
        if (this.isInWater()) {
            travelAIWater(strafe, vertical, forward);
        } else {
            super.travel(strafe, vertical, forward);
        }
    }

    private void travelAIWater(float strafe, float vertical, float forward) {
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand) {
        ...
        if (tryBreeding(player, stack)) return true;
        if (tryHealOrTame(player, stack)) return true;
        if (isOwner(player)) {
            if (handleStickSit(player, stack)) return true;
            if (handleGUIMount(player)) return true;
        }
        return super.processInteract(player, hand);
    }

    private boolean tryBreeding(EntityPlayer player, ItemStack stack) {
        if (stack.getItem() != Items.PRISMARINE_CRYSTALS || this.getGrowingAge() != 0 || isInLove()) return false;
    ...
    private boolean tryHealOrTame(EntityPlayer player, ItemStack stack) {
        if (stack.getItem() != Item.getItemFromBlock(Blocks.SPONGE) || stack.getMetadata() != 0) return false;
        if (this.getHealth() >= this.getMaxHealth() && this.isTamed()) return false;
        if (!player.capabilities.isCreativeMode) stack.shrink(1);
        this.heal(5);
        this.playSound(SoundEvents.ENTITY_GENERIC_EAT, 1, 1);
        spawnEatParticles(stack);
    ...
    private void spawnEatParticles(ItemStack stack) {
        if (this.world.isRemote) {
            for (int i = 0; i < 3; i++) {
                ParticleHelper.spawnParticle(this.world, EnumParticleTypes.ITEM_CRACK,
                    this.posX + (this.rand.nextFloat() * this.width * 2.0F) - this.width,
                    this.posY + this.rand.nextFloat() * this.height,
                    this.posZ + (this.rand.nextFloat() * this.width * 2.0F) - this.width,
                    0, 0, 0, Item.getIdFromItem(stack.getItem()), 0);
            }
        }
    }
    ...
    private boolean handleStickSit(EntityPlayer player, ItemStack stack) {
    ...
    private boolean handleGUIMount(EntityPlayer player) {
        if (player.isSneaking()) {
            this.openGUI(player);
            return true;
        }
        if (!this.world.isRemote && this.isSaddled() && !this.isChild() && !player.isRiding()) {
            this.setSitting(false);
