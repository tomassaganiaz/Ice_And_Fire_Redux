...
/** Cockatrice — bestia que petrifica con la mirada */
...
public class EntityCockatrice extends EntityTameable implements IAnimatedEntity, IBlacklistedFromStatues, IVillagerFear {
    ...
    private void syncTargetedEntityData() {
    ...
    private void applyBiteDamage() {
        ...
        if (this.getDistanceSq(this.getAttackTarget()) < 8) {
    ...
    private void applyJumpAttack() {
        if (this.getAnimation() != ANIMATION_JUMPAT || this.getAttackTarget() == null) return;
    ...
    private void applyLeapVelocity(double d0, double d1, float leap) {
        if ((double) leap >= 1.0E-4D) {
    ...
    private void updateSitProgress() {
    ...
    private void updateStareProgress() {
    ...
    private void applyStareEffects() {
        ...
        if (this.getAttackTarget() == null || shouldMelee()) return;
        if (!EntityGorgon.isEntityLookingAt(this, this.getAttackTarget(), VIEW_RADIUS) || !EntityGorgon.isEntityLookingAt(this.getAttackTarget(), this, VIEW_RADIUS)) return;
        if (!this.isStaring()) {
        ...
        handleTamingProgress();
    }

    private void handleTamingProgress() {
        if (this.isTamed() || !(this.getAttackTarget() instanceof EntityPlayer)) return;
        ...
        if (this.getTamingLevel() >= 1000) {
            ...
            if (this.getTamingPlayer() instanceof EntityPlayer)
    ...
    private void spawnStareParticles() {
        if (!this.world.isRemote || this.getTargetedEntity() == null || !this.isStaring()) return;
        if (!EntityGorgon.isEntityLookingAt(this, this.getTargetedEntity(), VIEW_RADIUS) || !EntityGorgon.isEntityLookingAt(this.getTargetedEntity(), this, VIEW_RADIUS)) return;
        if (!this.hasTargetedEntity()) return;
        ...
        double attackScale = (double) this.getAttackAnimationScale(0.0F);
        ...
        while (d4 < d3) {
            d4 += 1.8D - attackScale + this.rand.nextDouble() * (1.7D - attackScale);
