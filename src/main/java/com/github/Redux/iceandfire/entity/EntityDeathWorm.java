...
import com.github.Redux.iceandfire.entity.explosion.SandExplosion;
import com.github.Redux.iceandfire.entity.util.*;
import com.github.Redux.iceandfire.message.MessageDeathWormHitbox;
...
/** Gusano de la muerte — depredador del desierto */
...
public class EntityDeathWorm extends EntityTameable implements IBlacklistedFromStatues, IMultipartEntity, IAnimatedEntity, IVillagerFear, IAnimalFear, IPhasesThroughBlock {
    ...
    private void tryGrowOlder() {
        if (this.ticksExisted % 1000 != 0 || this.getWormAge() >= 5) return;
    ...
    private void spawnGrowthParticles() {
        for (int i = 0; i < 10 * this.getScaleForAge(); i++) {
            spawnHappyParticle(this.posX, this.posY, this.posZ);
            for (EntityMultipartPart segment : segments) {
                spawnHappyParticle(segment.posX, segment.posY, segment.posZ);
            }
        }
    }

    private void spawnHappyParticle(double x, double y, double z) {
        double px = x + (this.rand.nextFloat() * this.width * 2.0F) - this.width;
        double pz = z + (this.rand.nextFloat() * this.width * 2.0F) - this.width;
        int surfaceY = this.getSurface(MathHelper.floor(px), MathHelper.floor(y), MathHelper.floor(pz));
        ParticleHelper.spawnParticle(this.world, EnumParticleTypes.VILLAGER_HAPPY, px, surfaceY + 0.5F, pz, this.rand.nextGaussian() * 0.02D, this.rand.nextGaussian() * 0.02D, this.rand.nextGaussian() * 0.02D);
    }

    private void tryPounceAttack() {
    ...
    private void spawnSandExplosion() {
    ...
    protected void spawnSandBurrowParticles() {
        if (!this.isInSand()) {
            this.noClip = false;
            return;
        }
        IBlockState state = world.getBlockState(new BlockPos(this.posX, this.getSurface(MathHelper.floor(this.posX), MathHelper.floor(this.posY), MathHelper.floor(this.posZ)), this.posZ).down());
        int blockId = Block.getStateId(state);
        if (state.isOpaqueCube() && world.isRemote) {
            spawnBlockCrackParticle(this.posX, this.posY, this.posZ, this.width, true, blockId);
            for (EntityMultipartPart segment : segments) {
                spawnBlockCrackParticle(segment.prevPosX, segment.prevPosY, segment.prevPosZ, segment.width, true, blockId);
            }
        }
    ...
    protected void spawnRiddenSandParticles() {
        if (this.getControllingPassenger() == null) return;
        this.noClip = false;
        this.pushOutOfBlocks(this.posX, (this.getEntityBoundingBox().minY + this.getEntityBoundingBox().maxY) / 2.0D, this.posZ);
        if (isSandBelow() && world.isRemote) {
            int sandId = Block.getIdFromBlock(Blocks.SAND);
            spawnBlockCrackParticle(this.posX, this.posY, this.posZ, this.width, false, sandId);
            for (EntityMultipartPart segment : segments) {
                spawnBlockCrackParticle(segment.posX, segment.posY, segment.posZ, segment.width, false, sandId);
            }
        }
    }

    private void spawnBlockCrackParticle(double x, double y, double z, float width, boolean usePrevPos, int blockId) {
        double px = x + (this.rand.nextFloat() * width * 2.0F) - width;
        double pz = z + (this.rand.nextFloat() * width * 2.0F) - width;
        int surfaceY = this.getSurface(MathHelper.floor(usePrevPos ? x : px), MathHelper.floor(y), MathHelper.floor(usePrevPos ? z : pz));
        ParticleHelper.spawnParticle(this.world, EnumParticleTypes.BLOCK_CRACK, px, surfaceY + 0.5F, pz, this.rand.nextGaussian() * 0.02D, this.rand.nextGaussian() * 0.02D, this.rand.nextGaussian() * 0.02D, blockId);
    }
