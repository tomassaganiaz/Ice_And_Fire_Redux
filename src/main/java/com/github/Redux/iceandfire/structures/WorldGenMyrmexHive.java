...
/** WorldGenMyrmexHive — World Gen Myrmex Hive */
...
public class WorldGenMyrmexHive extends WorldGenerator {
    ...
    private void generateCircle(World world, Random rand, MyrmexHive hive, BlockPos position, int size, int height, EnumFacing direction) {
        generateCircleImpl(world, rand, hive, position, size, height, direction, true, false);
    }
    ...
    private void generateCircleRespectSky(World world, Random rand, MyrmexHive hive, BlockPos position, int size, int height, EnumFacing direction) {
        generateCircleImpl(world, rand, hive, position, size, height, direction, true, true);
    }
    ...
    private void generateCircleAir(World world, Random rand, MyrmexHive hive, BlockPos position, int size, int height, EnumFacing direction) {
        generateCircleImpl(world, rand, hive, position, size, height, direction, false, false);
    }

    private void generateCircleImpl(World world, Random rand, MyrmexHive hive, BlockPos position, int size, int height, EnumFacing direction, boolean placeResin, boolean respectSky) {
        IBlockState resin = jungle ? JUNGLE_RESIN : DESERT_RESIN;
        IBlockState sticky_resin = jungle ? STICKY_JUNGLE_RESIN : STICKY_DESERT_RESIN;
        int radius = placeResin ? size + 2 : size;
        if (placeResin) {
            for (float i = 0; i < radius; i += 0.5) {
                for (float j = 0; j < 2 * Math.PI * i; j += 0.5) {
                    ...
                    if (direction == EnumFacing.WEST || direction == EnumFacing.EAST) {
                        if (!respectSky || !world.canBlockSeeSky(position.add(0, x, z))) {
                            world.setBlockState(position.add(0, x, z), rand.nextInt(3) == 0 ? sticky_resin : resin, 2);
                        }
                    } else {
                        if (!respectSky || !world.canBlockSeeSky(position.add(x, z, 0))) {
    ...
    public void generateSphere(World world, Random rand, BlockPos position, int size, int height, IBlockState fill) {
        generateSphereImpl(world, rand, position, size, height, fill, fill, pos -> !world.isAirBlock(pos));
    }
    ...
    public void generateSphere(World world, Random rand, BlockPos position, int size, int height, IBlockState fill, IBlockState fill2) {
        generateSphereImpl(world, rand, position, size, height, fill, fill2, pos -> true);
    }
    ...
    public void generateSphereRespectResin(World world, Random rand, BlockPos position, int size, int height, IBlockState fill, IBlockState fill2) {
        generateSphereImpl(world, rand, position, size, height, fill, fill2, pos -> !world.isAirBlock(pos) || world.isAirBlock(pos) && !hasResinUnder(pos, world));
    }
    ...
    public void generateSphereRespectAir(World world, Random rand, BlockPos position, int size, int height, IBlockState fill, IBlockState fill2) {
        generateSphereImpl(world, rand, position, size, height, fill, fill2, pos -> !world.isAirBlock(pos));
    }

    private void generateSphereImpl(World world, Random rand, BlockPos position, int size, int height, IBlockState fill, IBlockState fill2, java.util.function.Predicate<BlockPos> canPlace) {
        int i2 = size;
        ...
        for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, -k, -l), position.add(j, k, l))) {
            if (blockpos.distanceSq(position) <= (double) (f * f * MathHelper.clamp(rand.nextFloat(), 0.75F, 1.0F)) && canPlace.test(blockpos)) {
                world.setBlockState(blockpos, fill2 == fill ? fill : rand.nextInt(3) == 0 ? fill2 : fill, 2);
            }
        }
    }
