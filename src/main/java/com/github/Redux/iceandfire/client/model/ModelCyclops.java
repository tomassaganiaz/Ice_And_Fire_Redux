...
public class ModelCyclops extends ModelDragonBase {
    ...
    public void animate(IAnimatedEntity entity, float f, float f1, float f2, float f3, float f4, float f5) {
        ...
        animator.endKeyframe();
        grabPose(15, -25, -0.5F, 5);
        grabPose(5, -48, -0.8F, 57);
        grabPose(5, -25, -0.5F, 5);
        animator.resetKeyframe(5);

        animator.setAnimation(EntityCyclops.ANIMATION_ROAR);
        roarKeyframe(-25, 0, 0, 5);
        for (int headYaw : new int[]{20, -20, 20, -20}) roarKeyframe(-45, headYaw, 45, 5);
        animator.resetKeyframe(5);
    ...
    private void walkLR(AdvancedModelRenderer right, AdvancedModelRenderer left, float speed, float degree, int offset, float offsetVal, float f, float f1) {
        this.walk(right, speed, degree, true, offset, offsetVal, f, f1);
        this.walk(left, speed, degree, false, offset, -offsetVal, f, f1);
    }

    private void flapLR(AdvancedModelRenderer left, AdvancedModelRenderer right, float speed, float degree, int offset, float offsetVal, float f2, float f1) {
        this.flap(left, speed, degree, true, offset, offsetVal, f2, f1);
        this.flap(right, speed, degree, false, offset, offsetVal, f2, f1);
    }

    private void roarKeyframe(int headX, int headY, int jawX, int duration) {
        animator.startKeyframe(duration);
        roarPose();
        this.rotate(animator, Head, headX, headY, 0);
        if (jawX != 0) this.rotate(animator, Jaw, jawX, 0, 0);
        animator.move(Head, 0, 0, -0.5F);
        animator.endKeyframe();
    }

    private void roarPose() {
        this.rotate(animator, body, 15, 0, 0);
    ...
    private void grabPose(int duration, int headX, float headY, int jawX) {
        animator.startKeyframe(duration);
        ...
        this.rotate(animator, Head, headX, 0, 0);
        animator.move(Head, 0, headY, 0);
        this.rotate(animator, Jaw, jawX, 0, 0);
        animator.endKeyframe();
    }
    ...
    public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5, EntityCyclops entity) {
        ...
        walkLR(this.rightleg, this.leftleg, speed_walk, degree_walk * -0.75F, 0, 0F, f, f1);
        walkLR(this.rightleg2, this.leftleg2, speed_walk, degree_walk * -0.5F, 1, -0.3F, f, f1);
        walkLR(this.rightarm, this.leftarm, speed_walk, degree_walk * -0.75F, 0, 0F, f, f1);
        walkLR(this.rightarm2, this.leftarm2, speed_walk, degree_walk * -0.5F, 1, -0.3F, f, f1);
        ...
        flapLR(this.leftarm, this.rightarm, speed_idle, degree_idle * -0.1F, 0, 0F, f2, 1);
        flapLR(this.leftarm2, this.rightarm2, speed_idle, degree_idle * -0.1F, 0, -0.1F, f2, 1);
