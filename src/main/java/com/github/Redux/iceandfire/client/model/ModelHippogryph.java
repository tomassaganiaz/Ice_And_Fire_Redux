...
public class ModelHippogryph extends ModelDragonBase {
	...
	private void hideSaddleParts() {
		for (AdvancedModelRenderer part : new AdvancedModelRenderer[]{
			NoseBand, ReinL, ReinR, ChestL, ChestR, Saddle, Saddleback,
			StirrupIronL, StirrupIronR, SaddleFront, StirrupL, StirrupR
		}) {
			part.isHidden = true;
		}
	}

	private void eatKeyframe(float jawRot, int duration) {
		...
		this.rotate(animator, Jaw, jawRot, 0, 0);
		animator.move(BackLegR1, 0, -2F, 0.5F);
		animator.move(BackLegR1_1, 0, -2F, 0.5F);
		animator.endKeyframe();
	}

	private void scratchKeyframe(float backLegR1Rot, float backLegR1_1Rot, int duration) {
		animator.startKeyframe(duration);
		...
		this.rotate(animator, BackLegR1, backLegR1Rot, 0, 0);
		this.rotate(animator, BackLegR1_1, backLegR1_1Rot, 0, 0);
		animator.endKeyframe();
	}

	private void bobLegs(AdvancedModelRenderer body, AdvancedModelRenderer leg1, AdvancedModelRenderer leg1_1, AdvancedModelRenderer thighR, AdvancedModelRenderer thighL, float speed, float degree, float f, float f1) {
		this.bob(body, speed, degree, false, f, f1);
		this.bob(leg1, -speed, degree, false, f, f1);
		this.bob(leg1_1, -speed, degree, false, f, f1);
		this.bob(thighR, -speed, degree, false, f, f1);
		this.bob(thighL, -speed, degree, false, f, f1);
	}
	...
	public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5, Entity entity) {
		...
		bobLegs(Body, BackLegR1, BackLegR1_1, HindThighR, HindThighL, speed_idle, degree_idle, f2, 1);
		...
		if (hippo.isFlying() || hippo.airBorneCounter > 50 || hippo.isHovering()) {
			...
			bobLegs(Body, BackLegR1, BackLegR1_1, HindThighR, HindThighL, speed_walk, degree_walk, f, f1);
