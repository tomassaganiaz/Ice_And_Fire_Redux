...
public class ModelCockatrice extends ModelDragonBase {
    ...
    public void animate(IAnimatedEntity entity, float f, float f1, float f2, float f3, float f4, float f5) {
        ...
        jumpPos();
        animator.endKeyframe();
        for (int z : new int[]{-75, -19, -75, -19, -75}) jumpArmKeyframe(z, 2);
        animator.resetKeyframe(5);
        animator.setAnimation(EntityCockatrice.ANIMATION_WATTLESHAKE);
        int[][] wattleSeq = {{3, 1, 0}, {2, 1, 1}, {3, -1, 0}, {2, -1, 1}, {3, 1, 0}, {2, 1, 1}};
        for (int[] w : wattleSeq) wattleKeyframe(w[0], w[1], w[2] == 1);
        animator.resetKeyframe(5);
    ...
    private void wattleKeyframe(int duration, int sign, boolean amplified) {
        animator.startKeyframe(duration);
        int w = amplified ? 33 : 23;
        int c2 = amplified ? 37 : 27;
        this.rotate(animator, neck, 0, 0, -23 * sign);
        this.rotate(animator, neck2, 0, 0, -13 * sign);
        this.rotate(animator, head, 5, 0, -15 * sign);
        this.rotate(animator, Comb1, 0, 0, -w * sign);
        this.rotate(animator, Comb2, 0, 0, -c2 * sign);
        this.rotate(animator, Wattle, 0, 0, -w * sign);
        animator.endKeyframe();
    }

    private void jumpArmKeyframe(int zRot, int duration) {
        animator.startKeyframe(duration);
        jumpPos();
        this.rotate(animator, leftUpperArm, 0, 30, zRot);
        this.rotate(animator, RightUpperArm, 0, -30, -zRot);
        animator.endKeyframe();
    }
    ...
    public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5, EntityCockatrice entity) {
        ...
        for (AdvancedModelRenderer arm : new AdvancedModelRenderer[]{leftUpperArm, RightUpperArm, leftLowerArm, RightLowerArm}) {
            this.walk(arm, speed_idle, degree_idle * 0.2F, false, 1, 0.1F, f2, 1);
        }
