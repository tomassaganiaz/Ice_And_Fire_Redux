...
/** Renderizador de Base Arrow */


public abstract class RenderBaseArrow<T extends EntityArrow> extends Render<T> {

	public RenderBaseArrow(RenderManager render) {
		super(render);
	}
	...
	public void doRender(T entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		for (int j = 0; j < 4; ++j) {
			...
			BufferBuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
			BufferBuilder.pos(-8.0D, -2.0D, 0.0D).tex(0.0F, f2).endVertex();
			BufferBuilder.pos(8.0D, -2.0D, 0.0D).tex(0.5F, f2).endVertex();
			BufferBuilder.pos(8.0D, 2.0D, 0.0D).tex(0.5F, f3).endVertex();
			BufferBuilder.pos(-8.0D, 2.0D, 0.0D).tex(0.0F, f3).endVertex();
			tessellator.draw();
		}
